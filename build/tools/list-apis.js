...
export function registerListApisTools(server) {
    server.tool("list_available_apis", "Lista todas las operaciones disponibles para una categoría específica.", {
        ...
        try {
            ...
            const formattedList = apis.map(api => `- ${api.operationId}: ${api.description}\n  Parámetros: ${api.parameters.join(', ')}`).join('\n');
            const content = `APIs disponibles en ${category}:\n\n${formattedList}`;
            return { content: [{ type: "text", text: content }] };
        }
        catch (error) {
            return createErrorResponse(`Error al listar APIs: ${error.message}`, toolName);
        }
    });
}
