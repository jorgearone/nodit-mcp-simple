...
export function registerApiCategoriesTools(server) {
    server.tool("list_api_categories", "Lista todas las categorías de APIs disponibles en tu servidor MCP personalizado.", {}, async () => {
        ...
        try {
            ...
            const formattedList = categories.map(category => `- ${category.name}: ${category.description}\n  Protocolos: ${category.supportedProtocols.join(', ')}`).join('\n');
            const content = `Categorías de APIs Disponibles:\n\n${formattedList}`;
            return { content: [{ type: "text", text: content }] };
        }
        catch (error) {
            return createErrorResponse(`Error al listar categorías: ${error.message}`, toolName);
        }
    });
}
